Feature: RetrieveEvents
	...
	Scenario: Retrieving commits from the server for events for all time 
		...
		And I add an event origin for the local machine as a header for the stream identified as 2157AC59-AD0D-4BF0-9CC1-238BDE2CEFB9 in the bucket identified as 'TestClient'
		...
		When I request events for synchronisation for the client 'TestClient' from the beggining of time
	...
	Scenario: Retrieving commits from the server for events that originate on another machine for all time 
		Given I have created a new event session
	...
	Scenario: Retrieving commits from the server for events that originate on another machine but not for all time
		...
		And I have created an event in the session with an id of E261A67D-2C00-4854-A0FF-6DEFA84A4276 for the stream identified as 1157AC59-AD0D-4BF0-9CC1-238BDE2CEFB9 in the bucket identified as 'TestClient'
		And I commit the session with the id DAD11DA9-64C6-4955-AF82-F12B66FBAF3B
		When I request events for synchronisation for the client 'TestClient' from 01-Jan-2015
		And I use the first commit in the event session
